...
**on a moving ship** a component cell is held by the ship rather than by the index, exactly as a carried machine is held by its ship rather than by MachineIndex — `StarshipTransport` follows every translate and rotate, and `cellsIn()` unions the carried cells over the grounded ones. nothing about a carried cell is persisted: it moves every tick, and the ship's own save already carries the blocks. ports needed none of this, because `portsIn()` was always rebuilt per pass off `Machine.activeMachines` for exactly this reason.
