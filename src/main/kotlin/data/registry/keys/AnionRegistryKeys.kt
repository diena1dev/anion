package dev.diena.anion.data.registry.keys

import dev.diena.anion.data.registry.AnionRegistryKey

object AnionRegistryKeys {
    object ANION_ITEM_REGISTRY : AnionRegistryKey("item_registry") {}
    object ANION_BLOCK_REGISTRY : AnionRegistryKey("block_registry") {}
    object ANION_FLUID_REGISTRY : AnionRegistryKey("fluid_registry") {}
    object ANION_GAS_REGISTRY : AnionRegistryKey("gas_registry") {}
    object ANION_ENERGY_REGISTRY : AnionRegistryKey("energy_registry") {}
}
